...
### GET /api/tts-openai-fm/options — TTS オプション一覧

Voice 一覧と Vibe 一覧を返す（フロントエンドのドロップダウン用）。

**レスポンス:**
```json
{"voices": ["alloy","ash","ballad","coral",...], "vibes": ["Playful","Calm",...]}
```

---

### POST /api/tts-openai-fm — TTS 音声合成 (openai-fm)

openai.fm API (`https://www.openai.fm/api/generate`) を使用してテキストを音声に変換する。

**リクエスト:**
```json
{"text": "こんにちは", "voice": "coral", "vibe": "Playful", "browser": false}
```

| パラメータ | デフォルト | 説明 |
|---|---|---|
| `text` | (必須) | 読み上げテキスト |
| `voice` | `"coral"` | 音声 (alloy, ash, ballad, coral, echo, fable, onyx, nova, sage, shimmer, verse) |
| `vibe` | `"Playful"` | 音声スタイル |
| `browser` | `false` | `true`: WAV バイナリを返却 (ブラウザ再生用)、`false`: デバイスで aplay 再生 |

**動作:**
1. Vibe 辞書からプロンプト文字列を構築
2. openai.fm API に multipart/form-data で送信
3. `browser: true` の場合 → WAV を `audio/wav` で返却
4. `browser: false` の場合 → `/tmp/tts_openai_fm.wav` に保存 → `aplay` で非同期再生 → `{"status":"ok"}` を返却
...
- **読み上げ (TTS):** チェックボックスで有効化。LLM 応答を openai-fm API で音声変換
  - **Voice:** 11種の音声から選択 (デフォルト: Coral)
  - **Vibe:** 12種のスタイルから選択 (デフォルト: Playful)
  - **ブラウザ再生:** ON でブラウザ側で再生、OFF でデバイスのスピーカー (aplay) で再生
