...
6. `aec_daemon` を起動し、`media` + `pet_voice` サービスを再起動
...
バックアップから `asound.conf` を復元し、`aec_daemon` を停止、`media` + `pet_voice` を再起動する。
...
2. リスニングチャイム (880Hz, 200ms) を再生して聞き取り状態を通知
3. ZMQ キューをフラッシュ (0.5秒) してウェイクワード残テキストを破棄
4. listening 状態に戻り、ユーザー発話を蓄積
5. 連鎖バージインにも対応（新しい TTS 中にさらにバージイン可能）

### テキスト蓄積 (listening フェーズ)

listening 状態（ウェイクワード検出またはバージイン後）では、ユーザー発話を重複排除しながら蓄積する:

- **ASR ストリーミング更新**: pet_voice はセグメント内でテキストを段階的に更新する。共通プレフィックス (3文字以上) がある場合は上書き、それ以外は新セグメントとして追加
- **UTTERANCE_GAP (2秒)**: 新しいテキストが2秒間来なければ発話終了と判定
- **ドレイン (1秒)**: ギャップ発火後、1秒間 ASR の最終更新をキャッチ
- 複数セグメントは「。」で結合して LLM に送信
...
| `_play_listening_chime()` | `aplay -D tts_out` で 880Hz チャイムを再生 (ブロッキング) |
